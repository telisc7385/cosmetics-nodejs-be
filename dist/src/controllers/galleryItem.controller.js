...
exports.deleteGalleryItem = exports.updateGalleryItem = exports.getAllGalleryItems = exports.createGalleryItem = void 0;
...
const createGalleryItem = async (req, res) => {
    ...
    try {
        const existingItem = await prisma_1.default.galleryItem.findFirst({
        ...
        if (section) {
            const type = await prisma_1.default.galleryType.findUnique({
                where: { name: section },
            });
            if (!type) {
                res.status(400).json({
        ...
        const result = await (0, uploadToCloudinary_1.uploadToCloudinary)(req.file.buffer, 'gallery');
...
const getAllGalleryItems = async (req, res) => {
    try {
        ...
        const [items, totalCount] = await Promise.all([
            ...
            prisma_1.default.galleryItem.count({
                where: Object.keys(whereClause).length ? whereClause : undefined,
            }),
        ]);
...
const updateGalleryItem = async (req, res) => {
    ...
    const existing = await prisma_1.default.galleryItem.findUnique({ where: { id } });
    if (!existing) {
    ...
    try {
        ...
        if (sequence_number !== undefined) {
            const duplicate = await prisma_1.default.galleryItem.findFirst({
                where: {
        ...
        const updated = await prisma_1.default.galleryItem.update({
            where: { id },
            data: {
