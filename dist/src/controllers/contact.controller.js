...
exports.createContactRequest = exports.deleteContactRequest = exports.markContactAsHandled = exports.getContactRequestById = exports.getContactRequests = void 0;
const prisma_1 = __importDefault(require("../db/prisma")); // adjust path
const client_1 = require("@prisma/client");
const contactUser_1 = require("../email/contactUser");
const contactAdmin_1 = require("../email/contactAdmin");
        ...
        const whereClause = search
            ? {
                OR: [
                    ...
                    { phone_number: { contains: search, mode: client_1.Prisma.QueryMode.insensitive } },
                ],
            }
            : {};
        const [totalCount, records] = await Promise.all([
            prisma_1.default.contactRequest.count({ where: whereClause }),
...
const getContactRequestById = async (req, res) => {
    ...
    try {
        const contact = await prisma_1.default.contactRequest.findUnique({ where: { id } });
        if (!contact) {
...
exports.getContactRequestById = getContactRequestById;
const markContactAsHandled = async (req, res) => {
    const customReq = req; // type cast here
    ...
    try {
        const updated = await prisma_1.default.contactRequest.update({
            where: { id },
            data: {
...
exports.markContactAsHandled = markContactAsHandled;
...
const createContactRequest = async (req, res) => {
    const customReq = req;
    const { name, email, phone_number, city, state, country, own_retail_space, subject, message, } = req.body;
    if (!name || !email || !phone_number) {
        res.status(400).json({ message: 'Name, email and phone number are required' });
        return;
    }
    try {
        const newContact = await prisma_1.default.contactRequest.create({
            data: {
                name,
                email,
                phone_number,
        ...
        await (0, contactUser_1.sendContactConfirmationToUser)(email, name);
        // Get all admin users
        const companyEmail = await prisma_1.default.companySettings.findFirst({
            // where: { isDeleted: false },
            select: { email: true },
        });
        if (companyEmail?.email) {
            await (0, contactAdmin_1.sendContactAlertToAdmin)(companyEmail.email, name, email, subject, message);
        }
        res.status(201).json({ message: 'Contact request created', contact: newContact });
    }
    catch (error) {
