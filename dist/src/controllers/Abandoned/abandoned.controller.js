...
exports.getUsersSpecificAbandonedItems = exports.applyAbandonedCartDiscount = exports.getAbandonedCartDiscount = void 0;
const prisma_1 = __importDefault(require("../../db/prisma"));
const library_1 = require("@prisma/client/runtime/library");
const getAbandonedCartDiscount = async (req, res) => {
    ...
    try {
        ...
        const abandonedItems = await prisma_1.default.abandonedCartItem.findMany({
            where: {
                cartId: cart.id,
                userId: cart.userId,
            },
        ...
        for (const cartItem of cart.items) {
            const match = abandonedItems.find((item) => item.productId === cartItem.productId &&
                item.variantId === cartItem.variantId &&
                item.quantity === cartItem.quantity);
            if (match) {
                const rawPrice = cartItem.variant?.selling_price ||
...
exports.getAbandonedCartDiscount = getAbandonedCartDiscount;
const applyAbandonedCartDiscount = async (req, res) => {
    ...
    try {
        ...
        const subtotal = cart.items.reduce((sum, item) => {
            ...
            const price = rawPrice instanceof library_1.Decimal ? rawPrice.toNumber() : Number(rawPrice);
...
exports.applyAbandonedCartDiscount = applyAbandonedCartDiscount;
const getUsersSpecificAbandonedItems = async (req, res) => {
...
exports.getUsersSpecificAbandonedItems = getUsersSpecificAbandonedItems;
