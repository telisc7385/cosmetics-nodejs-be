...
exports.duplicateBlog = exports.getBlogs = exports.deleteBlog = exports.toggleBlogActiveStatus = exports.updateBlog = exports.createblog = void 0;
const prisma_1 = __importDefault(require("../../db/prisma"));
const extractName_1 = require("../../utils/extractName");
const category_controller_1 = require("../CategoryControllers/category.controller");
const uploadToCloudinary_1 = require("../../utils/uploadToCloudinary");
const createblog = async (req, res) => {
    try {
        const { title, is_active, content, product_tag_id, author, image_alternate_text, seo_metadata, seo_title, publish_date, tagjoints, seofocuskeywordjoints, } = req.body;
        // Basic validation
        ...
        const tagIds = Array.isArray(tagjoints)
            ? tagjoints.map(Number)
            : typeof tagjoints === "string"
                ? JSON.parse(tagjoints)
                : [];
        const keywordIds = Array.isArray(seofocuskeywordjoints)
            ? seofocuskeywordjoints.map(Number)
            : typeof seofocuskeywordjoints === "string"
                ? JSON.parse(seofocuskeywordjoints)
                : [];
        // Validate tag IDs exist
        const existingTags = await prisma_1.default.frontend_blogtag.findMany({
            where: { id: { in: tagIds } },
            select: { id: true },
        });
        const validTagIds = existingTags.map((t) => t.id);
        // Validate keyword IDs exist
        const existingKeywords = await prisma_1.default.frontend_blogseofocuskeyword.findMany({
        ...
        if (req.files && "image" in req.files) {
            ...
            if (mainFile?.buffer) {
                try {
                    const result = await (0, uploadToCloudinary_1.uploadToCloudinary)(mainFile.buffer, "blogs");
        ...
        // Generate slug
        let baseSlug = (0, category_controller_1.generateCategorySlug)(title);
        let slug = baseSlug;
        let count = 1;
        while (await prisma_1.default.frontend_blog.findFirst({ where: { slug } })) {
            slug = `${baseSlug}-${count++}`;
        }
        // Create blog with relations
        const blog = await prisma_1.default.frontend_blog.create({
            data: {
                title,
                slug,
                content,
...
const updateBlog = async (req, res) => {
    try {
        ...
        const existingBlog = await prisma_1.default.frontend_blog.findUnique({
            where: { id: blogId },
        });
        if (!existingBlog) {
            res.status(404).json({
        ...
        if (title !== existingBlog.title) {
            ...
            slug = baseSlug;
            let count = 1;
            while (await prisma_1.default.frontend_blog.findFirst({
                where: {
                    slug,
                    NOT: { id: blogId },
                },
        ...
        await prisma_1.default.frontend_blogandtagjoint.deleteMany({
            where: { blog_id: blogId },
        });
        await prisma_1.default.frontend_blogandseofocuskeywordjoint.deleteMany({
            where: { blog_id: blogId },
        });
        // Now update blog with new data & relations
        const updatedBlog = await prisma_1.default.frontend_blog.update({
            where: { id: blogId },
            data: {
                title,
                slug,
...
const toggleBlogActiveStatus = async (req, res) => {
    try {
        ...
        const toBoolean = (val) => typeof val === 'boolean' ? val : String(val).toLowerCase() === 'true';
        const updatedBlog = await prisma_1.default.frontend_blog.update({
            where: { id: blogId },
            data: {
                is_active: toBoolean(is_active),
                updated_by: await (0, extractName_1.getUserNameFromToken)(req),
            },
        });
        res.status(200).json({
            success: true,
...
exports.toggleBlogActiveStatus = toggleBlogActiveStatus;
const deleteBlog = async (req, res) => {
    try {
        ...
        const blog = await prisma_1.default.frontend_blog.findUnique({
            where: { id: blogId },
        });
        if (!blog) {
            res.status(404).json({
        ...
        // Now delete the blog
        await prisma_1.default.frontend_blog.delete({
            where: { id: blogId },
        });
        res.status(200).json({
            success: true,
...
const getBlogs = async (req, res) => {
    try {
        const { is_active, page = "1", page_size = "10", ordering, search = "", tag_id } = req.query;
        ...
        const [blogs, total] = await Promise.all([
            prisma_1.default.frontend_blog.findMany({
                where,
                orderBy,
                skip,
                take,
            ...
            prisma_1.default.frontend_blog.count({ where }),
        ]);
...
const duplicateBlog = async (req, res) => {
    const blogId = parseInt(req.params.id);
    try {
        const original = await prisma_1.default.frontend_blog.findUnique({
            where: { id: blogId },
            include: {
                tagjoints: true,
                seofocuskeywordjoints: true,
        ...
        const username = await (0, extractName_1.getUserNameFromToken)(req);
        const duplicated = await prisma_1.default.frontend_blog.create({
            data: {
        ...
        if (original.tagjoints.length > 0) {
            await prisma_1.default.frontend_blogandtagjoint.createMany({
        ...
        if (original.seofocuskeywordjoints.length > 0) {
            await prisma_1.default.frontend_blogandseofocuskeywordjoint.createMany({
...
exports.duplicateBlog = duplicateBlog;
